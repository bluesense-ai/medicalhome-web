...
const MicProgressBar: React.FC<MicProgressBarProps> = ({ audioStream,setMicInactive }) => {
  ...
  const [isMicActive, setIsMicActive] = useState<boolean>(true); // Track microphone activity
  ...
  useEffect(() => {
    ...
    if (audioStream) {
      ...
      const updateVolume = () => {
        ...
        setVolumeLevel(averageVolume); // Update state with volume level
    ...
    <div>
      ...
      {/* <p
        className={`mt-2 text-sm font-semibold ${
          isMicActive ? "text-green-600" : "text-red-600"
        }`}
      >
        {isMicActive ? "Microphone Active" : "No Input Detected"}
      </p> */}
