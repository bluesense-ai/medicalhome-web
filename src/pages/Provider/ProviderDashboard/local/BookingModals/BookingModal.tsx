...
interface BookingModalProps {
  isOpen: boolean;
  onRequestClose: () => void;
  services: { id: string; name: string,defaultDuration?: number }[];
  selectedEvent: any;
...
const BookingModal: React.FC<BookingModalProps> = ({
 ...
 const [formValues, setFormValues] = useState<any>({
  ...
  const handleStaffChange = (selectedOptions: any) => {
    setFormValues({
      ...formValues,
      staffMemberIds: selectedOptions.map((option: any) => option.value),
    });
  }
  ...
  useEffect(() => {
    if (formValues.healthCardNumber && !isPopupOpen) {
      helper.checkExistingPatient(formValues, setFormValues);
    }
  }, [formValues.healthCardNumber]);

  useEffect(() => {
    helper.initializeFormValues(selectedEvent, setFormValues);
  }, [selectedEvent]);

  const updateBooking = async () => {
    ; // Debugging breakpoint for troubleshooting
  
    if (!selectedEvent || !selectedEvent.id) {
      console.error("Error: Selected event is missing or invalid.");
      return;
    }
    ...
    try {
      await bookingService.updateBooking({
        ...
        setModalIsOpen: onRequestClose,
        setLoading: setLoadingAvailability,
      });
  ...
  useEffect(() => {
    if (selectedSlot) {
      setFormValues({
        ...
        customerNotes: "",
        serviceId: "",
        staffMemberIds: [provider.providerID],
        customerPhone: "",
        customerEmailAddress: "",
        isCustomerAllowedToManageBooking: false,
        optOutOfCustomerEmails: false,
  ...
  useEffect(() => { 
    if (formValues.start && formValues.serviceId) {
  ...
  }, [formValues.start, formValues.serviceId]);
            ...
            <div className="flex flex-col w-full">
              ...
              <div className="flex justify-start items-center  gap-10">
                <input
                  ...
                  name="start"
                  value={commonHelper.formatDate(formValues.start)}
                  ...
                  <div className="col-span-1">
                    <div className="form-group">
                      <h4 className="text-black text-base font-normal font-['Roboto'] leading-tight tracking-wide mb-5">
                        ...
                        >
                          {commonHelper.formatTime(formValues.start)}
                        </span>
                      </h4>
                      {loadingAvailability && (
